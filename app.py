...
# --- HIGH-FIDELITY CAD GENERATION (CORRECTED ORIENTATION) ---
def generate_optimized_design1(w_dia, d_dia, lg, wires):
    ...
    # 3. Precision Ceramic Waist Core (Flat Horizontal Annular Disc with Radial Slots)
    ceramic_disc_thick = 3.0
    ceramic_z_pos = 18 + 4 + (lg / 2) - (ceramic_disc_thick / 2)
    
    ceramic_disc = (cq.Workplane("XY")
                    .circle(core_outer_r).extrude(ceramic_disc_thick)
                    .faces(">Z").workplane().circle(center_bore_r).cutThruAll())
    
    # Cut radial slots/notches for wire positioning on the ceramic disc
    slot_tool = cq.Workplane("XY").rect(0.6, core_outer_r * 2.2).extrude(ceramic_disc_thick)
    for i in range(12):
        rotated_slot = slot_tool.rotate((0,0,0), (0,0,1), i * 30)
        ceramic_disc = ceramic_disc.cut(rotated_slot)

    assy.add(ceramic_disc, name="CeramicWaistCore", loc=cq.Location(cq.Vector(0, 0, ceramic_z_pos)), color=cq.Color(0.15, 0.15, 0.15))
