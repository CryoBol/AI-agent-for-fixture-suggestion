...
def generate_optimized_design1(w_dia, d_dia, lg, wires):
    ...
    # 1. Base Stand (Heavy chamfered base per image_4f04c9.jpg)
    ...
    # Micro-perforated matrix for Nitinol wires
    rings = 3
    holes_per_ring = max(12, wires // rings)
    for r_idx in range(rings):
        current_rad = center_bore_r + 3 + (r_idx * ((core_outer_r - center_bore_r - 4) / rings))
        plate = plate.faces(">Z").workplane().polarArray(current_rad, 0, 360, holes_per_ring).circle(0.4).cutThruAll()

    assy.add(plate, name="BottomRetentionPlate", loc=cq.Location(cq.Vector(0, 0, 18)), color=cq.Color(0.8, 0.8, 0.85))
    
    # Top plate requires a thicker center boss for the thread shown in the reference
    top_plate = plate.faces(">Z").workplane().circle(center_bore_r + 4).extrude(3)
    assy.add(top_plate, name="TopRetentionPlate", loc=cq.Location(cq.Vector(0, 0, 18 + 4 + lg)), color=cq.Color(0.8, 0.8, 0.85))

    # 3. Precision Ceramic Waist Core with Radial Fins
    ...
    # Cut radial slots to create the finned ceramic look
    slot_tool = cq.Workplane("XY").rect(0.5, core_outer_r * 2.5).extrude(lg)
    for i in range(36):
        rotated_tool = slot_tool.rotate((0,0,0), (0,0,1), i * 10)
        core_profile = core_profile.cut(rotated_tool)
...
# --- TRIGGER EVALUATION ---
if st.button("🚀 Generate High-Fidelity Model & Render FEM", type="primary", width="stretch"):
    
    with st.spinner("Computing precision boolean matrices and tessellating geometry... (This may take 15-30 seconds)"):
        try:
            # 1. Generate CAD
            assy = generate_optimized_design1(waist_dia, disc_dia, length, wire_count)
            compound = assy.toCompound()
            
            # 2. Extract Tessellated Mesh directly from the CadQuery Solid
            ...
            # 3. Calculate Nodal Temperatures mapped precisely to the vertices
            # Hot in the center ceramic core, dissipating outwards into the heavy steel base
            z_mid = 22 + (length / 2)
            dist_from_center = np.sqrt(x**2 + y**2)
            
            # Simulated Thermal Penalty Equation
            T = temp - (np.abs(z - z_mid) * 0.4) - (dist_from_center * 0.15)
            
            st.success("Geometry and Nodal Thermal Mapping Computed Successfully.")
            
            # 4. Render exact CAD geometry mapped with FEM color gradient
            fig = go.Figure(data=[
                go.Mesh3d(
                    x=x, y=y, z=z,
                    i=i, j=j, k=k,
