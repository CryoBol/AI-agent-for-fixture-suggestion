...
st.set_page_config(page_title="PDA Occluder Heat Setting Fixture AI", layout="wide")

st.title("🔬 Asymmetric PDA Occluder Heat Setting Fixture Agent")
st.markdown("Parametric CAD generation and nodal thermal mapping tailored to exact PDA occluder geometry.")
...
# --- HIGH-FIDELITY ASYMMETRIC CAD GENERATION ---
def generate_pda_occluder_fixture(d_pulm, d_waist, d_aortic, lg, wires):
    center_bore_r = 3.0
    ...
    assy = cq.Assembly(name="PDA_Fixture_Assembly")
    ...
    # 2. Bottom Retention Plate (Pulmonary End Sizing)
    ...
    # Wire locating hole matrix matched to pulmonary diameter
    for r_idx in range(2):
        curr_r = center_bore_r + 2.0 + (r_idx * ((pulm_r - center_bore_r - 2) / 2))
    ...
    # 3. Top Retention Plate (Aortic Retention Disc Sizing)
    ...
    # Wire locating hole matrix matched to large aortic retention disc diameter
    for r_idx in range(3):
        curr_r = center_bore_r + 2.0 + (r_idx * ((aortic_r - center_bore_r - 2) / 3))
    ...
    # 4. Precision Asymmetric Core (Molding Pulmonary End -> Waist -> Aortic Disc Profile)
    ...
    assy.add(core_profile, name="PDA_AsymmetricCore", loc=cq.Location(cq.Vector(0, 0, 22)), color=cq.Color(0.15, 0.15, 0.15))

    # 5. Fasteners and Alignment Hardware
...
# --- TRIGGER EVALUATION ---
if st.button("🚀 Generate PDA Occluder Fixture & Render Thermal Profile", type="primary", use_container_width=True):
    
    with st.spinner("Computing asymmetric CAD geometry and mapping thermal gradients..."):
        try:
            # 1. Generate CAD Assembly
            assy = generate_pda_occluder_fixture(pulm_dia, waist_dia, aortic_dia, length, wire_count)
            ...
            st.success("PDA Occluder Fixture & Thermal Field Computed Successfully.")
            ...
            fig.update_layout(
                title=f"Asymmetric PDA Fixture Thermal Field (Target: {temp}°C)",
                scene=dict(
            ...
            # 5. Export STEP File
            filename = "PDA_Occluder_Fixture.step"
            cq.exporters.export(compound, filename)
            
            with open(filename, "rb") as file:
                st.download_button(
                    label="💾 Download PDA Fixture Manufacturing CAD (.STEP)",
                    data=file,
                    file_name=filename,
